...
class MLPs(nn.Module):
    ...
    def forward(self, ope_ma_adj_batch, ope_pre_adj_batch, ope_sub_adj_batch, batch_idxes, feats):
        ...
        ope_ma_adj_batch = ope_ma_adj_batch.to(device)
        ope_pre_adj_batch = ope_pre_adj_batch.to(device)
        ope_sub_adj_batch = ope_sub_adj_batch.to(device)
        batch_idxes = batch_idxes.to(device)
        h = (feats[1], feats[0], feats[0], feats[0])

        # Identity matrix for self-loop of nodes
        self_adj = torch.eye(feats[0].size(-2), dtype=torch.int64, device=device).unsqueeze(0).expand_as(
...
class HGNNScheduler(nn.Module):
    def __init__(self, model_paras):
        ...
        # and the element means the number of heads of each HGNN
        ...
        self.get_machines.append(GATedge((self.in_size_ope, self.in_size_ma), self.out_size_ma, self.num_heads[0],
                                         self.dropout, self.dropout, activation=F.elu, device=self.device))
    ...
    def get_action_prob(self, state, memories, flag_sample=False, flag_train=False):
        ...
        batch_idxes = state.batch_idxes.to(self.device)
        # Raw feats
        raw_opes = state.feat_opes_batch.to(self.device).transpose(1, 2)[batch_idxes]
        raw_mas = state.feat_mas_batch.to(self.device).transpose(1, 2)[batch_idxes]
        proc_time = state.proc_times_batch.to(self.device)[batch_idxes]
        # Normalize
        nums_opes = state.nums_opes_batch.to(self.device)[batch_idxes]
        ...
        for i in range(len(self.num_heads)):
            ...
            h_machine = self.get_machines[i](state.ope_ma_adj_batch, state.batch_idxes, features)
            features = (features[0], h_machine, features[2])
            # Second Stage, operation node embedding
            # shape: [len(batch_idxes), max(num_opes), out_size_ope]
            h_operation = self.get_operations[i](state.ope_ma_adj_batch, state.ope_pre_adj_batch,
                                                 state.ope_sub_adj_batch,
                                                 state.batch_idxes, features)
            features = (h_operation, features[1], features[2])

        # Stacking and pooling
        h_mas_pooled = h_machine.mean(dim=-2)  # shape: [len(batch_idxes), out_size_ma]
        ...
        if not flag_sample and not flag_train:
            ...
            for i in range(len(batch_idxes)):
                h_opes_pooled.append(torch.mean(h_operation[i, :nums_opes[i], :], dim=-2))
            h_opes_pooled = torch.stack(h_opes_pooled)  # shape: [len(batch_idxes), d]
        else:
            h_opes_pooled = h_operation.mean(dim=-2)  # shape: [len(batch_idxes), out_size_ope]

        # Detect eligible O-M pairs (eligible actions) and generate tensors for actor calculation
        ope_step_batch = torch.where(state.ope_step_batch.to(self.device) > state.end_ope_biases_batch.to(self.device),
                                     state.end_ope_biases_batch.to(self.device),
                                     state.ope_step_batch.to(self.device)).to(self.device)
        jobs_gather = ope_step_batch[..., :, None].expand(-1, -1, h_operation.size(-1))[batch_idxes].to(self.device)
        h_jobs = h_operation.gather(1, jobs_gather)
        # Matrix indicating whether processing is possible
        # shape: [len(batch_idxes), num_jobs, num_mas]
        eligible_proc = state.ope_ma_adj_batch.to(self.device)[batch_idxes].gather(1,
                                                                                   ope_step_batch[..., :, None].expand(
                                                                                       -1, -1,
                                                                                       state.ope_ma_adj_batch.size(-1))[
                                                                                       batch_idxes]).to(self.device)
        h_jobs_padding = h_jobs.unsqueeze(-2).expand(-1, -1, state.proc_times_batch.size(-1), -1).to(self.device)
        h_mas_padding = h_machine.unsqueeze(-3).expand_as(h_jobs_padding)
        ...
        # shape: [len(batch_idxes), num_jobs, num_mas]
        ma_eligible = ~state.mask_ma_procing_batch.to(self.device)[batch_idxes].unsqueeze(1).expand_as(
            h_jobs_padding[..., 0])
        # Matrix indicating whether job is eligible
        # shape: [len(batch_idxes), num_jobs, num_mas]
        job_eligible = ~(state.mask_job_procing_batch.to(self.device)[batch_idxes] +
                         state.mask_job_finish_batch.to(self.device)[batch_idxes])[:, :, None].expand_as(
            h_jobs_padding[..., 0])
        # shape: [len(batch_idxes), num_jobs, num_mas]
    ...
    def act(self, state, memories, dones, flag_sample=True, flag_train=True):
        ...
        machines = (action_indexes / state.mask_job_finish_batch.size(1)).long()
        jobs = (action_indexes % state.mask_job_finish_batch.size(1)).long()
        operations = ope_step_batch[state.batch_idxes, jobs]
...
class PPO:
    def __init__(self, model_paras, train_paras, num_envs=None):
        self.lr = train_paras["lr"]  # learning rate 学习率
        self.betas = train_paras["betas"]  # default value for Adam (Adam 优化器的 β 值)
        self.gamma = train_paras["gamma"]  # discount factor (折扣因子)
        self.eps_clip = train_paras["eps_clip"]  # clip ratio for PPO (PPO 裁剪比率)
        self.K_epochs = train_paras["K_epochs"]  # Update policy for K epochs (K 次迭代次数)
        self.A_coef = train_paras["A_coeff"]  # coefficient for policy loss (策略损失系数)
        self.vf_coef = train_paras["vf_coeff"]  # coefficient for value loss (值函数损失系数)
        self.entropy_coef = train_paras["entropy_coeff"]  # coefficient for entropy term (熵项系数)
        ...
        # 使用PyTorch的torch.optim.Adam 创建优化器，用于策略网络的参数更新
        self.optimizer = torch.optim.Adam(self.policy.parameters(), lr=self.lr, betas=self.betas)
        # 使用PyTorch的nn.MSELoss作为均方误差损失函数
        self.MseLoss = nn.MSELoss()

        # === 新增：保存一份冻结的预训练策略，用于 KL 正则 ===
        self.policy_pretrained = copy.deepcopy(self.policy).eval()
        for p in self.policy_pretrained.parameters():
            p.requires_grad = False

        # KL 系数，可以通过 train_paras 配置
        self.kl_coef = train_paras.get("kl_coef", 0.05)
        # self.kl_coef = 0.01

    def update(self, memory, env_paras, train_paras):
        device = env_paras["device"]
        mini_batch_size = train_paras["minibatch_size"]  # batch size for updating

        # Ensure all tensors inside memory are moved to the correct device
        old_ope_ma_adj = torch.stack([item.to(device) for item in memory.ope_ma_adj], dim=0).transpose(0, 1).flatten(0,
                                                                                                                     1)
        old_ope_pre_adj = torch.stack([item.to(device) for item in memory.ope_pre_adj], dim=0).transpose(0, 1).flatten(
            0, 1)
        old_ope_sub_adj = torch.stack([item.to(device) for item in memory.ope_sub_adj], dim=0).transpose(0, 1).flatten(
            0, 1)
        old_raw_opes = torch.stack([item.to(device) for item in memory.raw_opes], dim=0).transpose(0, 1).flatten(0, 1)
        old_raw_mas = torch.stack([item.to(device) for item in memory.raw_mas], dim=0).transpose(0, 1).flatten(0, 1)
        old_proc_time = torch.stack([item.to(device) for item in memory.proc_time], dim=0).transpose(0, 1).flatten(0, 1)
        old_jobs_gather = torch.stack([item.to(device) for item in memory.jobs_gather], dim=0).transpose(0, 1).flatten(
            0, 1)
        old_eligible = torch.stack([item.to(device) for item in memory.eligible], dim=0).transpose(0, 1).flatten(0, 1)
        memory_rewards = torch.stack([item.to(device) for item in memory.rewards], dim=0).transpose(0, 1)
        memory_is_terminals = torch.stack([item.to(device) for item in memory.is_terminals], dim=0).transpose(0, 1)
        old_logprobs = torch.stack([item.to(device) for item in memory.logprobs], dim=0).transpose(0, 1).flatten(0, 1)
        old_action_envs = torch.stack([item.to(device) for item in memory.action_indexes], dim=0).transpose(0,
                                                                                                            1).flatten(
            0, 1)
        ...
        num_complete_mini_batches = math.floor(full_batch_size / mini_batch_size)
        # Optimize policy for K epochs:
        for _ in range(self.K_epochs):
            for i in range(num_complete_mini_batches + 1):
                if i < num_complete_mini_batches:
                ...
                ratios = torch.exp(logprobs - old_logprobs[i * mini_batch_size:(i + 1) * mini_batch_size].detach())
                advantages = rewards_envs[i * mini_batch_size:(i + 1) * mini_batch_size] - state_values.detach()
                surr1 = ratios * advantages
                surr2 = torch.clamp(ratios, 1 - self.eps_clip, 1 + self.eps_clip) * advantages
                loss = - self.A_coef * torch.min(surr1, surr2) \
                       + self.vf_coef * self.MseLoss(state_values,
                                                     rewards_envs[i * mini_batch_size:(i + 1) * mini_batch_size]) \
                       - self.entropy_coef * dist_entropy
                # === 新增：KL 正则约束，防止偏离预训练策略太远 ===
                with torch.no_grad():
                    pretrained_logprobs, _, _ = self.policy_pretrained.evaluate(
                ...
                # 预训练策略概率
                pretrained_probs = torch.exp(pretrained_logprobs)

                # KL 散度项
                kl_div = torch.sum(pretrained_probs * (pretrained_logprobs - logprobs), dim=-1).mean()
