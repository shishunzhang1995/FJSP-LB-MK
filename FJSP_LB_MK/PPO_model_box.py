...
def build_ope_ok_tensor(job_ok: torch.Tensor, ope_step_batch: torch.Tensor):
    """
    job_ok: [B, J, 1]          # bool
    ope_step_batch: [B, J]     # int
    return: [B, J, 1]          # 填好 -1 的 ope_ok tensor
    """
    B, J, _ = job_ok.shape

    # 初始化结果为 -1
    ope_ok_tensor = torch.full((B, J), fill_value=-1, dtype=ope_step_batch.dtype)

    # 去掉 job_ok 最后一维以做广播匹配
    job_ok_mask = job_ok.squeeze(-1)  # [B, J], bool

    # 对 mask 为 True 的位置填入 ope_step
    ope_ok_tensor[job_ok_mask] = ope_step_batch[job_ok_mask]

    # 恢复 shape 为 [B, J, 1]
    return ope_ok_tensor.unsqueeze(-1)

def get_valid_third_opes_tensor_batchwise(ope_ok_padded, ma_ok, third_opes):
    """
    修复版：逐 batch 判断每个 batch 是否满足条件，再单独处理
    - ope_ok_padded: [B, J, 1]
    - ma_ok: [B, J]           # 每个 job 的最后一台机器是否可用
    - third_opes: [B, 10]     # 每个 batch 的 third ope 集合
    """
    B, J, _ = ope_ok_padded.shape
    result = torch.full_like(ope_ok_padded, fill_value=-1)
    ope_flat = ope_ok_padded.squeeze(-1)   # [B, J]
    # print("ma_ok=",ma_ok)

    for b in range(B):
        valid_ope = ope_flat[b]            # [J]
        valid_mask = valid_ope != -1
        valid_opes = valid_ope[valid_mask]  # 有效工序编号

        # batch 内单独判断
        is_third = torch.isin(valid_opes, third_opes[b])
        count_third = is_third.sum().item()

        # 使用最后一个 job 的 ma_ok 判断最后机器是否可用
        cond1 = count_third >= 2
        cond2 = ma_ok[b,0, -1]
        # print("cond1=",cond1)
        # print("cond2=", cond2)
        if cond1 and cond2:
            # 找出哪些位置的有效工序是 third_opes
            third_mask = torch.isin(valid_ope, third_opes[b]) & valid_mask  # [J]
            result[b, third_mask, 0] = valid_ope[third_mask]

    return result  # [B, J, 1]

def compute_fenjian_ope_selection_probs(
    filtered_ope_ok,
    matrix_part_similarity,
    last_fenjian_ope_batch,
    greedy=False,
    threshold=0.4
):
    """
    参数：
    - filtered_ope_ok: [B, J, 1]，候选分拣工序编号（-1 表示不可用）
    - matrix_part_similarity: [B, N, N]，ope之间的零件相似度
    - last_fenjian_ope_batch: [B]，上一个分拣工序编号
    - greedy: 是否用贪婪策略选最大值
    - threshold: 最小相似度阈值，低于此值的工序被忽略

    返回：
    - probs_list: List[Tensor]，每个 batch 的候选工序概率分布（或 one-hot）
    - indices_list: List[Tensor]，每个 batch 的被考虑的候选工序编号
    """
    B = filtered_ope_ok.shape[0]
    filtered = filtered_ope_ok.squeeze(-1)     # [B, J]
    probs_list = []
    indices_list = []

    for b in range(B):
        candidates = filtered[b]
        valid_mask = candidates != -1
        valid_opes = candidates[valid_mask]  # 当前 batch 中候选工序编号

        if valid_opes.numel() >= 2:
            last_ope = last_fenjian_ope_batch[b]
            similarity_row = matrix_part_similarity[b, last_ope]  # [N]
            similarity_values = similarity_row[valid_opes]        # [K]

            # 新增：按相似度阈值筛选
            threshold_mask = similarity_values >= threshold
            if threshold_mask.sum().item() >= 1:
                filtered_opes = valid_opes[threshold_mask]
                filtered_sim = similarity_values[threshold_mask]

                if greedy:
                    max_idx = torch.argmax(filtered_sim)
                    one_hot = torch.zeros_like(filtered_sim)
                    one_hot[max_idx] = 1.0
                    probs_list.append(one_hot)
                else:
                    probs = F.softmax(filtered_sim, dim=0)
                    probs_list.append(probs)

                indices_list.append(filtered_opes)
                continue

        # 不满足条件：候选数不足或无高相似度者
        probs_list.append(torch.tensor([]))
        indices_list.append(torch.tensor([]))

    return probs_list, indices_list
...
class MLPs(nn.Module):
    ...
    def forward(self, ope_ma_adj_batch,
                ope_pre_adj_batch, ope_sub_adj_batch, batch_idxes, feats,ope_box_adj_batch):
        '''
        ...
        '''
        h = (feats[1], feats[0], feats[0], feats[0],feats[3])
        ...
        adj = (ope_ma_adj_batch[batch_idxes], ope_pre_adj_batch[batch_idxes],
               ope_sub_adj_batch[batch_idxes], self_adj,ope_box_adj_batch[batch_idxes])
        MLP_embeddings = []
        # print("feats[3].shape=",feats[3].shape) #torch.Size([20, 12, 1])
        # print("adj_ope_box[batch_idxes].shape=",adj_ope_box[batch_idxes].shape) #torch.Size([20, 49, 1])
        # print("feats[1].shape=", feats[1].shape)  # torch.Size([20, 6, 8])
        # print("ope_ma_adj_batch[batch_idxes].shape=", ope_ma_adj_batch[batch_idxes].shape)
        for i in range(len(adj)):
            MLP_embeddings.append(self.gnn_layers[i](h[i], adj[i]))
        MLP_embedding_in = torch.cat(MLP_embeddings, dim=-1)
        # print("MLP_embedding_in.shape=", MLP_embedding_in.shape) ##torch.Size([20, 50, 40])
...
class HGNNScheduler(nn.Module):
    def __init__(self, model_paras):
        ...
        self.in_size_box= model_paras["in_size_box"]
        ...
        self.get_operations.append(MLPs([self.out_size_ma, self.in_size_ope, self.in_size_ope, self.in_size_ope,self.in_size_box],
                                        self.hidden_size_ope, self.out_size_ope, self.num_heads[0], self.dropout))
        for i in range(len(self.num_heads)-1):
            self.get_operations.append(MLPs([self.out_size_ma, self.out_size_ope, self.out_size_ope, self.out_size_ope,self.in_size_box],
    ...
    '''
    def get_normalized(self, raw_opes, raw_mas,feat_boxes, proc_time, batch_idxes, nums_opes, flag_sample=False, flag_train=False):
        '''
        ...
        if not flag_sample and not flag_train:
            ...
            mean_box = torch.mean(feat_boxes, dim=-2, keepdim=True)
            std_mas = torch.std(raw_mas, dim=-2, keepdim=True)
            std_box = torch.std(feat_boxes, dim=-2, keepdim=True)
    ...
    def get_action_prob(self, state, memories, flag_sample=False, flag_train=False):
        ...
        batch_idxes = state.batch_idxes  ### uncompleted instance for train: [0,..19]; for test [0]
        # Raw feats
        raw_opes = state.feat_opes_batch.transpose(1, 2)[batch_idxes]
        # print("raw_opes=",raw_opes)
        # print("raw_opes.size()=", raw_opes.size()) #torch.Size([20, 48, 20])
        raw_mas = state.feat_mas_batch.transpose(1, 2)[batch_idxes]
        # print("raw_mas.size()=", raw_mas.size())  #torch.Size([20, 6, 3])
        feat_boxes = state.feat_box_batch.unsqueeze(-2)
        # print("feat_boxes.size()=",feat_boxes.size())
        proc_time = state.proc_times_batch[batch_idxes]
        # Normalize
        nums_opes = state.nums_opes_batch[batch_idxes] ## the number of operations for each instance
        features = self.get_normalized(raw_opes, raw_mas, feat_boxes,proc_time, batch_idxes, nums_opes, flag_sample, flag_train)
        norm_opes = (copy.deepcopy(features[0]))
        norm_mas = (copy.deepcopy(features[1]))
        norm_proc = (copy.deepcopy(features[2]))
        norm_box=(copy.deepcopy(features[3]))
        # print("norm_box.size()=",norm_box.size())
        # print("norm_box=",norm_box)
        # adj_ope_box=torch.ones((raw_opes.shape[0], raw_opes.shape[1], 1), dtype=torch.float32)  # OPE → Box
        # adj_box_ope = adj_ope_box.transpose(1, 2)  # Box → OPE，如果要双向传播
        ...
        for i in range(len(self.num_heads)):
            ...
            features = (features[0], h_mas, features[2],features[3]) ## update feature with embedded machine feature
            # Second Stage, operation node embedding
            # shape: [len(batch_idxes), max(num_opes), out_size_ope]
            # print("state.ope_box_adj_batch=",state.ope_box_adj_batch)
            h_opes = self.get_operations[i](state.ope_ma_adj_batch, state.ope_pre_adj_batch, state.ope_sub_adj_batch,
                                            state.batch_idxes, features,state.ope_box_adj_batch)
            features = (h_opes, features[1], features[2],features[3]) ## update feature with embedded operation feature
        ...
        # print("eligible_proc=",eligible_proc.size())
        h_jobs_padding = h_jobs.unsqueeze(-2).expand(-1, -1, state.proc_times_batch.size(-1), -1)
        # print("h_jobs=", h_jobs)  ## 10 x 8
        # print("h_jobs_padding.shape=", h_jobs_padding.shape)  ## 5 x 10 x 8

        h_mas_padding = h_mas.unsqueeze(-3).expand_as(h_jobs_padding)
        # print("h_mas=", h_mas) ## 5 x 8
        # print("h_mas_padding=", h_mas_padding)  ## 10 x 5 x 8
        ...
        # print("h_mas_pooled_padding=", h_mas_pooled_padding)
        # print("h_opes_pooled_padding=", h_opes_pooled_padding)
        # Matrix indicating whether machine is eligible
        # shape: [len(batch_idxes), num_jobs, num_mas]
        # print("state.mask_ma_procing_batch.shape=",state.mask_ma_procing_batch.shape)
        ma_eligible = ~state.mask_ma_procing_batch[batch_idxes].unsqueeze(1).expand_as(h_jobs_padding[..., 0])

        # print("ma_eligible=",ma_eligible.size())
        ...
        # print("~(state.mask_job_procing_batch[batch_idxes]+state.mask_job_finish_batch[batch_idxes])[:, :, None]",
        #       ~(state.mask_job_procing_batch[batch_idxes] +
        #         state.mask_job_finish_batch[batch_idxes])[:, :, None]
        #       )
        # print("job_eligible=",job_eligible.size())
        # shape: [len(batch_idxes), num_jobs, num_mas]
        # print("ma_eligible=", ma_eligible)
        # print("job_eligible=", job_eligible)
        job_ok=~(state.mask_job_procing_batch[batch_idxes] +
                         state.mask_job_finish_batch[batch_idxes])[:, :, None]
        # print(job_ok.shape)
        # print(ope_step_batch.shape)
        mask = job_ok.squeeze(-1)  # shape: [1, 10], bool

        eligible = job_eligible & ma_eligible & (eligible_proc == 1)  ## 10x5  (job x ma)
        # print("eligible=", eligible.size())
        ...
        h_actions = torch.cat((h_jobs_padding, h_mas_padding, h_opes_pooled_padding, h_mas_pooled_padding),
                              dim=-1).transpose(1, 2)
        # print("h_actions=",h_actions)
        # print("h_actions.size()=",h_actions.size()) ### torch.Size([20, 5, 10, 32])
        h_pooled = torch.cat((h_opes_pooled, h_mas_pooled), dim=-1)  # deprecated  1 x 16 (8+8)
        # print("h_pooled=", h_pooled)
        mask = eligible.transpose(1, 2).flatten(1)
        # print("mask=",mask)  ## 50 ()

        # Get priority index and probability of actions with masking the ineligible actions
        scores = self.actor(h_actions).flatten(1)  ## 1x50
        # print("scores.size()=", scores.size()) #torch.Size([20, 50])
        scores[~mask] = float('-inf')

        for i in range(len(batch_idxes)):
            if (~mask[i]).all():
                scores[i]=0*len(scores[i])

        # print("scores=",scores.size())
        # print("scores=",scores)
        action_probs = F.softmax(scores, dim=1)
        # print("action_probs=", action_probs)
        ## firstly prob equal for every eligible O-M pairs, so pick randomly from this O-M
        ## first random is also so random !!
        # print("action_probs.size()=", action_probs.size()) #torch.Size([20, 50])
        # print("scores=", scores)

        # Store data in memory during training
        if flag_train == True:
            memories.ope_ma_adj.append(copy.deepcopy(state.ope_ma_adj_batch))
            memories.ope_box_adj.append(copy.deepcopy(state.ope_box_adj_batch))
            ...
            memories.feat_boxes.append(copy.deepcopy(feat_boxes))
    ...
    def act(self, state, memories, dones, flag_sample=True, flag_train=True):
        ...
        action_probs, ope_step_batch, _ = \
            self.get_action_prob(state, memories, flag_sample, flag_train=flag_train)
        # print("action_probs=",action_probs)
        # print("action_probs=",action_probs)
        # print("action_probs.size=", action_probs.size())  ## 1x50 (50= 10 jobs x 5 machine)
        # print("ope_step_batch=", ope_step_batch)
        #num_ope_biases= [0, 5, 11, 17, 21, 25, 30, 36, 42, 48]
        #ope_step_batch= tensor([[ 0,  5, 11, 17, 21, 25, 30, 36, 42, 48]])
        # print("ope_step_batch.size=", ope_step_batch.size())
        ...
        # print("state.mask_ma_procing_batch.size(1)=",state.mask_ma_procing_batch.size(1))
        all_opes=copy.deepcopy(opes)
        # print("probs_list=",probs_list)
        # print("indices_list=",indices_list)
        # print("ope_step_batch=",ope_step_batch)
    ...
    ## only used in train
    def evaluate(self, ope_ma_adj,ope_box_adj, ope_pre_adj, ope_sub_adj, raw_opes, raw_mas, feat_boxes,proc_time,
                 jobs_gather, eligible, action_envs, flag_sample=False):
        batch_idxes = torch.arange(0, ope_ma_adj.size(-3)).long()
        features = (raw_opes, raw_mas, proc_time,feat_boxes)

        # L iterations of the HGNN
        for i in range(len(self.num_heads)):
            h_mas = self.get_machines[i](ope_ma_adj, batch_idxes, features)
            features = (features[0], h_mas, features[2],features[3])
            h_opes = self.get_operations[i](ope_ma_adj,ope_pre_adj, ope_sub_adj,
                                            batch_idxes, features,ope_box_adj)
        ...
        h_actions = torch.cat((h_jobs_padding, h_mas_padding, h_opes_pooled_padding, h_mas_pooled_padding),
                              dim=-1).transpose(1, 2)
        # print("h_actions.size()=",h_actions.size())
...
class PPO:
    ...
    def update(self, memory, env_paras, train_paras):
        ...
        old_ope_box_adj=torch.stack(memory.ope_box_adj, dim=0).transpose(0,1).flatten(0,1)
        ...
        # print("raw_opes=",memory.raw_opes)
        # print("len(raw_opes)=", len(memory.raw_opes))
        # print("raw_opes[0]=", memory.raw_opes[0])
        ...
        old_feat_boxes=torch.stack(memory.feat_boxes,dim=0).transpose(0, 1).flatten(0, 1)
            ...
            for i in range(num_complete_minibatches+1):
                ...
                logprobs, state_values, dist_entropy = \
                    self.policy.evaluate(old_ope_ma_adj[start_idx: end_idx, :, :],
                                         old_ope_box_adj[start_idx: end_idx, :, :],
                                         ...
                                         old_feat_boxes[start_idx: end_idx, :, :],
