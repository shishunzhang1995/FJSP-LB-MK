...
class GATedge(nn.Module):
    ...
    def forward(self, ope_ma_adj_batch, batch_idxes, feat):
        ...
        ope_ma_adj_batch = ope_ma_adj_batch.to(self.device)
        batch_idxes = batch_idxes.to(self.device)
        feat = feat.to(self.device) if isinstance(feat, torch.Tensor) else tuple(f.to(self.device) for f in feat)
        ...
        mask = torch.cat((
                        (ope_ma_adj_batch[batch_idxes].unsqueeze(-1) == 1),
                        torch.full(
                            size=(ope_ma_adj_batch[batch_idxes].size(0), 1, ope_ma_adj_batch[batch_idxes].size(2), 1),
                            dtype=torch.bool,
                            fill_value=True,
                            device=self.device  # Ensure this tensor is on the same device
                        )
                    ), dim=-3)
