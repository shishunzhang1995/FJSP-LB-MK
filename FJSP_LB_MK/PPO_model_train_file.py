...
class HGNNScheduler(nn.Module):
    ...
    def get_action_prob(self, state, memories, flag_sample=False, flag_train=False):
        ...
        # feat_boxes = state.feat_box_batch.unsqueeze(-1)
        ...
        for i in range(len(self.num_heads)):
            ...
            features = (features[0], h_mas, features[2]) ## update feature with embedded machine feature
            ...
            features = (h_opes, features[1], features[2]) ## update feature with embedded operation feature
        ...
        ope_ok = ope_step_batch[mask]  # shape: [num_valid_jobs]

        ope_ok_padded = build_ope_ok_tensor(job_ok, ope_step_batch)
        ma_ok=~state.mask_ma_procing_batch[batch_idxes].unsqueeze(1)
        # print("state.num_ope_batch=",state.num_ope_batch)
        # print("state.num_ope_batch.shape=", state.num_ope_batch.shape)
        # print("state.nums_opes_batch=",state.nums_opes_batch)
        # print("state.num_ope_biases_batch=",state.num_ope_biases_batch)
        start_indices=state.num_ope_biases_batch
        third_opes = start_indices + 3
        # print("ope_ok_padded=",ope_ok_padded)
        # print("ope_ok_padded.shape=", ope_ok_padded.shape)
        # print("third_opes=", third_opes)
        # print("third_opes.shape=",third_opes.shape)
        filtered_ope_ok = get_valid_third_opes_tensor_batchwise(ope_ok_padded, ma_ok, third_opes)
        # print("filtered_ope_ok=",filtered_ope_ok)
        # print("state.last_fenjian_ope_batch=",state.last_fenjian_ope_batch)
        # # print("state.last_fenjian_ope_batch.shape=", state.last_fenjian_ope_batch.shape)
        # # print("state.matrix_part_similarity=",state.matrix_part_similarity)
        # print("state.matrix_part_similarity.shape=", state.matrix_part_similarity.shape)
        # print("filtered_ope_ok=",filtered_ope_ok)
        probs_list, indices_list = compute_fenjian_ope_selection_probs(
            filtered_ope_ok,
            state.matrix_part_similarity,
            state.last_fenjian_ope_batch,
            greedy=False, #或 True
            threshold=1 ##重叠度: 0.6; 新增类别数: 0.8
        )

        # print("job_eligible=",job_ok)
        ...
        return action_probs, ope_step_batch, h_pooled,probs_list, indices_list
    ...
    def act(self, state, memories, dones, flag_sample=True, flag_train=True):
        ...
        action_probs, ope_step_batch, _,probs_list, indices_list = \
        ...
        for b, (probs, all_opes) in enumerate(zip(probs_list, indices_list)):

            if probs.numel() > 0:
                # print(f"Batch {b}: 候选工序 = {all_opes.tolist()}, 概率分布 = {probs.tolist()}")
                ### greedy
                max_idx = torch.argmax(probs)  # 在候选列表中的索引
                ##samling
                # max_idx = torch.multinomial(probs, num_samples=1).item()
                best_ope = all_opes[max_idx].item()  # 概率最高的 ope 编号

                opes[b] = best_ope  # 保存该 batch 的最优 ope
                mas[b]=state.mask_ma_procing_batch.size(1)-1
                jobs[b]=state.opes_appertain_batch[b][opes[b]]
        # print("ope_step_batch=",ope_step_batch)
